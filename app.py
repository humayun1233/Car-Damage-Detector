# I am using this os library beacsue of creating & removing directory
import os
# Retrieve file/ pathname
import glob
# classes predcition
from classify import prediction
# convert the regular code to tensorflow grpah.pb file
import tensorflow as tf
# all the wrok load will be shifted on cpu not on gpu
import  _thread
import time
...
def predict():
    ...
    if (human_string == 'car'):
        label_text = 'This is not a damaged car . Please upload a damaged car image'
        print(image_path)
        return render_template('front.html', text = label_text, filename= image_path)
    elif (human_string == 'low'):
        label_text = 'This is a low damaged car'
        print(image_path)
        return render_template('front.html', text = label_text, filename= image_path)
    elif (human_string == 'high'):
        label_text = 'This is a high damaged car'
        print(image_path)
        return render_template('front.html', text = label_text, filename= image_path)
    elif (human_string == 'not'):
        label_text = 'This is not the image of a car. Please upload the car image.'
        print(image_path)
        return render_template('front.html', text = label_text, filename= image_path)
